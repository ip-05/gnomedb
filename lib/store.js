...
class DataStore {
        ...
        if (typeof subQuery === 'object') {
          ...
          for (const subQueryKey of subQueryKeys) {
            if (subQueryKey === '$gte') {
              ...
              vertices = vertices ? this.intersection(vertices, records) : this.union(equalTo.data, greaterThan);
            } else if (subQueryKey == '$gt') {
              const greaterThan = index.findByKey(subQuery[subQueryKey]).getChildren();
              vertices = vertices ? this.intersection(vertices, greaterThan) : greaterThan;
            }
          }
        } else {
  ...
  add(doc) {
    ...
    for (const [key, index] of this.indeces) {
