...
class BinarySearchTree {
  ...
  addFrom(obj) {
    ...
    if (!obj || type !== 'object') throw new Error('Collection of Objects was expected!');

    const length = Object.keys(obj).length || obj.length; // object or array
    if (!length) throw new Error('Object must have a non-zero length!');
    ...
    const removeData = (data, node, prevNode = null) => {
      ...
      if (node.key === data[this.fieldKey]) {
        if (node.data.has(data)) {
          ...
          if(!node.data.size) { //  нужно менять дерево если дата пустая
